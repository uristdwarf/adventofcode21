...
out:
	for _, num := range numbers {
		for boardIndex, board := range boards {
			...
			if board.CheckRow() || board.CheckCol() {
				lastNum = num
				if len(boards) > 1 {
					boards = RemoveElement(boards, boardIndex)
					goto out
				} else {
					break out
				}
	...
	GetLoserBoard(lastBoard, boards[lastBoard], lastNum)
}

func GetLoserBoard(boardIndex int, board Board, num string) {
...
func RemoveElement(s []Board, i int) []Board {
